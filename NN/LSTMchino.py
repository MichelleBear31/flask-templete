...
def train_model(train_dir, model_path='LSTM_model.keras', history_path='LSTM_history.pkl', epochs=1500, batch_size=128):
...
def main():
    ...
    if not os.path.exists(model_path) or not os.path.exists(history_path):
        try:
            history, categories = train_model(output_dir, model_path=model_path, history_path=history_path, epochs=1500, batch_size=128)
